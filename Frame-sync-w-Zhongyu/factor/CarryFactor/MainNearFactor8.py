...
class MainNearFactor8(BaseCarryFactor):
    """
    【Cautions】: 由于计算因子没有做标准化，该因子的意义很模糊，应用前景受限
    ...
    计算方法: log(近月合约价格) - log(远月合约价格)
    由于没有做 月份或者天数的标准化操作，横截面上的比较变得意义模糊
    ...
    def compute_single_factor(self, symbol: str) -> DataFrame:
        ...
        factor = (np.log(near_far_df[near_price]) - np.log(near_far_df[far_price]))
...
if __name__ == "__main__":
    self = MainNearFactor8(price='close')
