...
class BasisMomentum3(BaseCarryFactor):
    """
    ...
    计算方法: (近月合约区间收益-远月合约区间收益)/月份差

    用于 TS2（近月、主力）、TS4（主力、次主力） pair only 的原始因子设计月化处理  区别于 BasisMomentum1 的 pair 非月化处理
    ...
    def compute_single_factor(self, symbol: str) -> DataFrame:
        ...
        factor = (near_far_df['interval_ret_near'] - near_far_df['interval_ret_far'])* 12 / (near_far_df['abs_month_far'] - near_far_df['abs_month_near'])
...
if __name__ == "__main__":
    self = BasisMomentum3(price='close', R=20)
