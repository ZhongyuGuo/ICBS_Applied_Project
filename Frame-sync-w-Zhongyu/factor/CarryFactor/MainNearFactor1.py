#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2021/6/22 13:58:15
# @Author  : Michael_Liu @ QTG
# @File    : MainNearFactor3
# @Software: PyCharm
...
class MainNearFactor1(BaseCarryFactor):
    """
    利用【主力合约】和去除主力合约后的如下合约 (不允许进交割月, 且 交易量大于 0) 计算的期限结构因子
        最近合约： others = ‘near’
        最远合约:  others = 'far'
        次主力合约: others = 'second'
    ...
    See Also: MainNearFactor4.py, 只是展期收益率的具体计算方式不同
    ...
    def compute_single_factor(self, symbol: str) -> DataFrame:
        ...
        near_contract = near_contract.reset_index(). \
            merge(daily_data[['datetime', 'contract', price]], on=['datetime', 'contract'])

        far_contract = far_contract.reset_index(). \
            merge(daily_data[['datetime', 'contract', price]], on=['datetime', 'contract'])

        near_far_df = pd.merge(near_contract, far_contract, on='datetime',
                               suffixes=('_near', '_far')).set_index('datetime')
        ...
        factor = ((np.log(near_far_df[near_price]) - np.log(near_far_df[far_price])) * 12/
...
if __name__ == "__main__":
    self = MainNearFactor1(price='close')
