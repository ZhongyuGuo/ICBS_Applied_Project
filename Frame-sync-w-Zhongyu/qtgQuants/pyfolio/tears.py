...
def create_returns_tear_sheet(returns, positions=None,
    ...
    fig = plt.figure(figsize=(14, vertical_sections * 6))
    gs = gridspec.GridSpec(vertical_sections, 2, wspace=0.5, hspace=0.5)
    ax_rolling_returns = plt.subplot(gs[:2, :])
    ...
    ax_monthly_returns = plt.subplot(gs[i, 1])
    i+=1
    ax_annual_returns = plt.subplot(gs[i, 0])
    ...
    plotting.plot_monthly_returns(returns, ax=ax_monthly_returns)
